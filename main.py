...
# TO DO!! make funcs: admin kayboard and users keyboard
...
def keyboard(flag):
    ...
    check_balance_button = types.KeyboardButton(text='/balance нажмите сюда, чтобы узнать свой баланс')
    help_button = types.KeyboardButton(text='/help  нажмите сюда, чтобы узнать о боте')
    keyboard.add(buy_button, check_balance_button, help_button)
    if flag:
        keyboard.add(types.KeyboardButton('/start - назад'))
    return keyboard
...
def help_command(message):
    logging(message)
    global keyboard
    bot.send_message(message.chat.id, 'bot description', reply_markup=keyboard(1))
